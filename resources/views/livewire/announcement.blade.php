<div>
    <x-hero class="text-black" backgroundImage="{{ asset('images/announcement.jpg') }}" />
    ...
    <x-container>
        ...
        <div class="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($posts as $post)
                <div>
                    <a class="*:transition group" href="{{ $post->url }}" wire:navigate>
                        <div class="w-full h-48 mb-2 overflow-hidden border rounded-lg group-hover:brightness-90">
                            @if ($post->image)
                                <img class="object-cover w-full h-full" src="{{ $post->image->url }}"
